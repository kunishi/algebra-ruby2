...
class TestFiniteGroup < Test::Unit::TestCase
  ...
  def test_conjugacy_classes # test for conjugacy_classes
    assert_equal(@S3.conjugacy_classes.map_s(&:size),
                 Set[1, 3, 2])
    assert_equal(@S4.conjugacy_classes.map_s(&:size),
                 Set[6, 6, 8, 3, 1])
    assert_equal(@S5.conjugacy_classes.map_s(&:size),
  ...
  def test_commutator_series # test for commutator_series
    assert_equal(@A4.commutator_series.collect(&:size), [12, 4, 1])
    assert_equal(@S5.commutator_series.collect(&:size), [120, 60])
  end
  ...
  def test_descending_central_series # test for descending_central_series
    assert_equal(@A4.descending_central_series.collect(&:size), [12, 4])
    assert_equal(@S5.descending_central_series.collect(&:size), [120, 60])
  end
  ...
  def test_ascending_central_series # test for ascending_central_series
    assert_equal(@A3.ascending_central_series.collect(&:size), [1])
    assert_equal(@S3.ascending_central_series.collect(&:size), [1])
    assert_equal(@A4.ascending_central_series.collect(&:size), [1])
    assert_equal(@S4.ascending_central_series.collect(&:size), [1])
  end
