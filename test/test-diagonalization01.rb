...
class TestDiagonalization01 < Test::Unit::TestCase
  ...
  def test_diagonalization01
    ...
    e = a.diagonalize
    assert_equal('t^3 - t^2 + t - 6/1', e.chpoly.to_s)
    assert_equal('(t - 2/1)(t^2 + t + 3/1)', e.facts.to_s)
    ...
    e.roots.each do |po, rs|
      ...
      assert_equal('t^2 + t + 3/1', po.to_s)
    ...
    assert_equal('[4/1, -5/1, 1/1]', e.espaces[evs[0]].map(&:to_s).join(', '))
    assert_equal('r(-11/4) - 1/2', (evs[1]).to_s)
    assert_equal('[1/3r(-11/4) + 1/6, 1/3r(-11/4) + 1/6, 1/1]', e.espaces[evs[1]].map(&:to_s).join(', '))
    assert_equal('-r(-11/4) - 1/2', (evs[2]).to_s)
    assert_equal('[-1/3r(-11/4) + 1/6, -1/3r(-11/4) + 1/6, 1/1]', e.espaces[evs[2]].map(&:to_s).join(', '))
